...
import { PLANNER_TASKS, MOCK_WEATHER } from './lookahead/lookaheadConstants';
import { LookaheadTask, Constraint, ConstraintStatus, ConstraintType, WeatherForecast } from './lookahead/lookaheadTypes';
import ConstraintBadge from './lookahead/ConstraintBadge';
import ManHoursBar from './lookahead/ManHoursBar';
import LookaheadDetailsPanel from './lookahead/LookaheadDetailsPanel';
import SparklineChart from './lookahead/SparklineChart';
import { ChevronDownIcon, ChevronRightIcon, DocumentIcon, SunIcon, CloudIcon, CloudRainIcon } from './Icons';
...
const getDaysDiff = (d1: Date, d2: Date) => Math.round((d2.getTime() - d1.getTime()) / (1000 * 3600 * 24));
...
const LookaheadView: React.FC = () => {
    ...
    const { projectStartDate, projectEndDate, totalDays } = useMemo(() => {
        ...
        const start = allTasks.reduce((min, t) => parseDate(t.startDate) < min ? parseDate(t.startDate) : min, parseDate(allTasks[0].startDate));
        const end = allTasks.reduce((max, t) => parseDate(t.finishDate) > max ? parseDate(t.finishDate) : max, parseDate(allTasks[0].finishDate));
    ...
    const weatherByDate = useMemo(() => new Map(MOCK_WEATHER.map(w => [w.date, w])), []);
            ...
            <div className="flex-grow overflow-hidden relative flex">
                <div className="flex-grow overflow-auto">
                    <div className="relative" style={{ minWidth: `${LEFT_PANEL_WIDTH + (totalDays * DAY_WIDTH)}px`}}>
                        {/* Header */}
                        <div className="sticky top-0 bg-gray-50 z-20 text-xs font-semibold text-gray-600 uppercase border-b border-gray-200">
                            <div className="flex" style={{ height: '30px' }}>
                             ...
                             <div className="flex" style={{ height: '50px' }}>
                                 <div className="sticky left-0 bg-gray-50 flex border-r border-gray-200" style={{ width: `${LEFT_PANEL_WIDTH}px` }}>
                                    <div className="w-[50px] flex-shrink-0 px-2 flex items-center justify-center">ID</div>
                                    <div className="w-[250px] flex-shrink-0 px-2 flex items-center border-l border-gray-200">Task Name</div>
                                    <div className="w-[100px] flex-shrink-0 px-2 flex items-center border-l border-gray-200">Resource</div>
                                    <div className="w-[175px] flex-shrink-0 px-2 flex items-center border-l border-gray-200">Constraint Health</div>
                                    <div className="w-[175px] flex-shrink-0 px-2 flex items-center border-l border-gray-200">Man-Hours</div>
                                 </div>
                                 <div className="flex-grow flex">
                                    {Array.from({length: totalDays}).map((_, i) => {
                                        ...
                                        return (
                                            <div key={i} className={`flex flex-col items-center justify-between py-1 border-r border-gray-200 ${isWeekend ? 'bg-gray-100' : ''}`} style={{width: `${DAY_WIDTH}px`}}>
