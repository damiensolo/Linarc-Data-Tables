...
import { AssigneeAvatar, ImpactPill, PrioritySelector } from './TaskElements';
...
const BoardView: React.FC<{ tasks: Task[]; onPriorityChange: (taskId: number, priority: Priority) => void; }> = ({ tasks, onPriorityChange }) => {
                ...
                <div key={status} className="w-80 bg-gray-100 rounded-lg flex-shrink-0 flex flex-col">
                    ...
                    <div className="p-1 overflow-y-auto flex-grow">
                        {tasksByStatus[status].map(task => (
                            <TaskCard key={task.id} task={task} onPriorityChange={onPriorityChange} />
                        ))}
                    </div>
                </div>
            ))}
